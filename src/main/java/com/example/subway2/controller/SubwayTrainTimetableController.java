...
import com.example.subway2.repository.SubwayTrainTimetableRepository;
        ...
        try {
            for(StationCode tempIdx : stationCodes)
            {
                for(String day : dayCd) {
                    ...
                    String urlStr = "http://openapi.kric.go.kr/openapi/trainUseInfo/subwayTimetable?serviceKey=$2a$10$Mj4bgn36lc9Eb1ODdU6fbuQuDHmeNVGNzVF1gcBqnNRNXqLD2onT." +
                            "&format=json" +
                            "&railOprIsttCd=" + tempIdx.getRailOprIsttCd() +
                            "&dayCd=" + day +
                            "&lnCd=" + tempIdx.getLnCd() +
                            "&stinCd=" + tempIdx.getStinCd();
                    ...
                    if (array != null) {
                        for (int i = 0; i < array.size(); i++) {
                            ...
                            SubwayTrainTimetable subwayTrainTimetable = new SubwayTrainTimetable(
                            ...
                            subwayTrainTimetableRepository.save(subwayTrainTimetable);
                            numofTuples++;
                        }
                    }
                }
